...
export default function WorksheetViewPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const [prog, comp] = await Promise.all([
      getStudentWorksheetProgress(user.id, id),
  ...
  const locked = isLockedByDerived(completions, lessonKeys.worksheet(worksheet.id), allDone);

  if (locked) {
    const totalCoins = worksheet.tasks.reduce((s, t) => s + t.coins, 0);
    return (
      <div className="min-h-screen bg-[var(--terminal-bg)] text-[var(--terminal-primary)]">
        <div className="max-w-3xl mx-auto p-4">
          <button
            onClick={() => router.push("/worksheets")}
            className="text-sm text-[var(--terminal-primary-dim)] hover:text-[var(--terminal-primary)] mb-4 block"
          >
            ← 返回學習單列表
          </button>

          <div className="border-2 border-green-600 bg-green-900/10 p-6 text-center">
            <div className="text-5xl mb-3">🎉</div>
            <h1 className="text-2xl font-bold text-green-400 mb-1">這張學習單完成了！</h1>
            <div className="text-sm text-[var(--terminal-primary-dim)]">
              {worksheet.title} · {worksheet.semester} W{String(worksheet.week).padStart(2, "0")}
            </div>
            <div className="text-lg text-yellow-400 font-bold mt-4">
              🪙 {progress?.totalCoinsAwarded || 0} / {totalCoins} 金幣
            </div>
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
            {worksheet.tasks.map((task) => {
              const tp = progress?.tasks?.[task.taskId];
              return (
                <div key={task.taskId} className="p-3 border border-green-700 bg-green-900/20 text-center">
                  <div className="font-bold text-sm text-green-400 mb-1">
                    ✓ 任務 {task.taskId}
                  </div>
                  <div className="text-xs text-[var(--terminal-primary-dim)] truncate" title={task.label}>
                    {task.label}
                  </div>
                  <div className="text-xs text-green-400 mt-1">+{tp?.coinsAwarded ?? 0} 金幣</div>
                  {tp?.approverName && (
                    <div className="text-[10px] text-[var(--terminal-primary-dim)] mt-1">
                      {tp.approverName} 認證
                    </div>
                  )}
                </div>
          ...
          <div className="mt-6 mb-12 p-4 border border-[var(--terminal-primary-dim)] text-center text-sm text-[var(--terminal-primary-dim)]">
            已經完成的學習單不能重做囉。如果需要再做一次，請老師幫你開放。
          </div>
        </div>
      </div>
    );
