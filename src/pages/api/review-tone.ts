...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const RUBRIC: Record<string, string> = {
    溫柔:
      "放慢、給對方空間、先安撫情緒。\n" +
      "【特別注意】溫柔不代表不能問問題。像「別怕，沒關係，等你準備好再告訴我」" +
      "裡的『等你準備好再說／再告訴我』就是在給對方空間、讓他不急著回答，" +
      "這是很好的溫柔，一定要通過。只有『快講、快點說、到底看到什麼』這種" +
      "逼人馬上回答的才算催。不要把溫柔的邀請誤讀成催促。",
    認真:
...
3. 標準放在小三程度：只要語氣方向對、講得通，就通過。不要求文采、不要求標點完美、
   不要求字數多。太嚴格會把小孩卡住。
4. 【傾向通過】只有語氣「明顯相反」或「明顯缺席」才不通過——
   例如要溫柔卻在兇人、要有禮貌卻在命令、亂打一通。
   如果句子的語氣方向是對的、只是不夠完美，一律通過，不要挑剔。
   拿不定主意（覺得「好像可以又好像差一點」）時，一律通過。

feedback 規則（很重要）：
- 40 字以內，講給小三聽。太長會被截斷。
- 不通過時要**具體說出哪裡不對**，不要只說「再試一次」。
  講完哪裡不對，可以再給一句範例句讓他參考（學生會自己重打一遍，那也是練習）。
  ✅ 好（示範格式，內容要照實際句子講，不要照抄這句）：「你直接命令老師了，改成問問看他方不方便。」
  ❌ 壞：「不夠溫柔，請再試一次。」← 沒講哪裡不對
